...
// 食材個数（種類別）
// ================================
function calcIngredientCountDetail(pokemon, level, pattern) {
...
function calcDailyEnergy(pokemon, level, subs, pattern, up, down) {
  ...
  // 食材
  const ingredientCountDetail = calcIngredientCountDetail(pokemon, level, pattern);
  let ingredientEnergy = 0;

  for (const [name, count] of Object.entries(ingredientCountDetail)) {
    ingredientEnergy += count * calcIngredientEnergyPerUnit(name);
  }
...
function displayResult(r, name) {
  ...
  let ingDetailHtml = "";
  for (const [name, count] of Object.entries(r.ingredientCountDetail)) {
    ingDetailHtml += `・${name}：${count.toFixed(2)} 個 / 日<br>`;
  }
