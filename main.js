...
async function loadDB() {
  ...
  // berry_base は JSON（{ "フシギバナ": 24, ... }）想定
  berryEnergyDB = await (await fetch("berry_base")).json();
...
// きのみ計算
// ================================
function getBerryEnergyByPokemonName(pokemonName) {
  // berry_base は「ポケモン名 → きのみ1個のエナジー」想定
  return Number(berryEnergyDB[pokemonName] ?? 0);
}
...
function calcDailyEnergy(pokemon, level, subs, pattern, up, down) {
  ...
  const berryEnergyPerOne = getBerryEnergyByPokemonName(pokemon.name);
